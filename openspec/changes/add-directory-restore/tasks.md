...
- [x] 1.1 Add `void RemoveExtractedFile(string absolutePath)` to `IContentStore`
...
- [x] 1.2 Implement `RemoveExtractedFile` in `FileSystemContentStore`
...
- [x] 2.1 Add `DirectoryRestoreEntry` and `DirectoryRestorePlan` records to
...
- [x] 2.2 Generalize `SnapshotHistoryService`'s existing directory-prefix helpers
...
- [x] 2.3 Implement `SnapshotHistoryService.PlanDirectoryRestore(directoryPath, asOf, outRoot,
...
- [x] 2.4 Implement `SnapshotHistoryService.ExecuteDirectoryRestore(plan, onBytesCopied,
...
- [x] 3.1 Add a `--recursive` boolean option to `RestoreCommand`
...
- [x] 3.2 Wire `--recursive` to call `PlanDirectoryRestore`/`ExecuteDirectoryRestore` instead of
...
- [x] 3.3 Implement the single confirmation prompt: reporting `plan.ToWrite.Count` and
...
- [x] 3.4 Wire progress reporting: construct one `RestoreProgressReporter` per directory
...
- [x] 3.5 Update `RestoreCommand`'s option/argument help text (`path`, new `--recursive`) to
...
- [x] 4.1 Confirm `ErrorReporting.cs`'s friendly-message switch already covers
...
- [x] 5.1 Add an integration test (`Vara.IntegrationTests`) that backs up a directory across
...
- [x] 5.2 Add an integration test for `--recursive --in-place` asserting removal of a
...
- [x] 5.3 Run the full test suite (`dotnet test` from `src`/repo root, per the project's existing
