...
- [ ] 1.1 In `SqliteSnapshotRepository.GetStateAsOf`, bind the `$asOf` SQL parameter using
...
- [ ] 1.2 In `SqliteSnapshotRepository.GetTombstones`, apply the same `ToIso(asOf.ToUniversalTime())`
  change to its `$asOf` binding.

## 2. Add regression coverage for non-UTC offsets

- [ ] 2.1 In `SqliteSnapshotRepositoryTests.cs`, add a test that records a `file_versions` row at a
  ...
  returns.
- [ ] 2.2 Add the equivalent test for `GetTombstones` with a non-zero-offset `asOf`, verifying
  deleted-entry inclusion/exclusion matches the same-instant `TimeSpan.Zero` result.
- [ ] 2.3 Add a test asserting `GetStateAsOf`/`GetTombstones` agree with
...
- [ ] 3.1 Run `dotnet test tests/Vara.Infrastructure.Tests` (or the full solution test run per the
