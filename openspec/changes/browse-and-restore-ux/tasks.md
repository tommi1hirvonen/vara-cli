...
- [x] 1.1 Make `AbsolutePathMirrorMapper` (`src/Vara.Infrastructure/FileSystem/
...
- [x] 1.2 Add a new `SnapshotPathResolver` in `src/Vara.Application/History/` implementing the
      ...
      no-match case.
- [x] 1.3 Wire `HistoryCommand` and `RestoreCommand` (`src/Vara.Cli/Commands/`) to resolve their
...
- [x] 2.1 Add a working-directory resolution path to `ProfileResolver`
...
- [x] 2.2 Update `HistoryCommand`, `RestoreCommand`, and the new `BrowseCommand`/
...
- [x] 2.3 Register the new "profile name required" exception in `ErrorReporting.cs`'s
      ...
      **No code change needed**: `ProfileNameRequiredException` derives from
      `ProfileConfigException`, already matched by the switch's `ProfileConfigException` case;
      verified manually (see task 2.2's manual verification) that it produces the friendly
      `Error: No profile name was given...` message.

## 3. Repository: point-in-time and tombstone queries

- [x] 3.1 Add `IReadOnlyDictionary<string, CurrentFileState> GetStateAsOf(DateTimeOffset asOf)`
...
- [x] 3.2 Add `IReadOnlyList<FileVersionRecord> GetTombstones(DateTimeOffset? asOf)` to
...
- [x] 3.3 Add `IReadOnlyDictionary<string, string> GetMoveOrigins()` to
...
- [x] 4.1 Add `Stream OpenRead(string hash)` to `IContentStore`
...
- [x] 4.2 Implement `OpenRead` on the in-memory `FakeContentStore`
...
- [x] 5.1 Add `IReadOnlyList<DirectoryEntry> ListDirectory(string directoryPath, DateTimeOffset?
...
- [x] 5.2 Add `IReadOnlyList<FileVersionRecord> ListDeleted(string? directoryPath,
...
- [x] 5.3 Add `void ShowVersion(string relativePath, long? versionId, DateTimeOffset? asOf,
...
- [x] 5.4 Add a way to open two resolved versions' content for comparison (e.g. `(Stream Left,
...
- [x] 6.1 Add `BrowseCommand` (`src/Vara.Cli/Commands/BrowseCommand.cs`) with an optional profile
...
- [x] 6.2 Add `DeletedCommand` (`src/Vara.Cli/Commands/DeletedCommand.cs`) with an optional
      ...
      and a manual run.
- [x] 6.3 Add `ShowCommand` (`src/Vara.Cli/Commands/ShowCommand.cs`) with an optional profile
...
- [x] 6.4 Add `DiffCommand` (`src/Vara.Cli/Commands/DiffCommand.cs`) with an optional profile
...
- [x] 6.5 Update `RestoreCommand` (`src/Vara.Cli/Commands/RestoreCommand.cs`): make `--out`
...
- [x] 6.6 Add the interactive version picker to `RestoreCommand`: when neither `--version` nor
...
- [x] 6.7 Register `BrowseCommand`, `DeletedCommand`, `ShowCommand`, `DiffCommand` in
      `src/Vara.Cli/Program.cs`. Verify `vara --help` lists all four new commands.

## 7. Presentation

- [x] 7.1 Add `DirectoryListingPresenter` (`src/Vara.Cli/Presentation/
...
- [x] 7.2 Add rendering for `ListDeleted`'s results (a `DeletedReportPresenter`, or reusing
...
- [x] 8.1 Add `BrowseCommandTests`/`DeletedCommandTests`/`ShowCommandTests`/`DiffCommandTests`,
      ...
      **Adapted to match this project's actual established CLI-testing boundary**: per the
      archived `restore-overwrite-guard` change's design.md ("keeps `RestoreCommand`'s own new
      code... as thin, manually-verifiable glue"), no existing command class in this codebase is
      tested via `System.CommandLine` parsing/invocation (`BackupCommandTests.cs` only tests the
      pure `ResolveOutcome` helper) - `StandardError.Console`/`AnsiConsole.Console` are also
      fixed static instances, not test-injectable. Added instead: `DirectoryListingPresenterTests`
      and `DeletedReportPresenterTests` (`tests/Vara.Cli.Tests/Presentation/`, following
      `HistoryTablePresenterTests.cs`'s `TestConsole`-based pattern) and
      `DirectoryArgumentResolverTests` (`tests/Vara.Cli.Tests/Commands/`) for the new,
      independently testable CLI-layer logic. `RestoreCommand`'s `--out`/`--in-place` mutual
      exclusivity, the interactive-picker fallback, and every new command's option parsing were
      manually verified end-to-end (see this session's manual verification runs) - consistent
      with how `RestoreCommand`'s existing `--force`/prompt logic is verified today.
- [x] 8.2 Add integration coverage under `tests/Vara.IntegrationTests/` exercising a real backup
      ...
      the new tests pass.
- [x] 8.3 Run `dotnet test` for the full solution and verify all tests pass, including every new
      case from sections 1-8.
- [x] 8.4 Manually validate the end-to-end scenarios from proposal.md: browsing a directory with
      ...
      nor `--at` is given.
      Verified live against a real profile (real backup run, real SQLite manifest, real content
      store): `browse <dir> --deleted` correctly interleaved a deleted `old.txt` (marked
      "Deleted") among live entries; `deleted --profile smoke` listed it profile-wide with its
      deletion timestamp; `restore <path> --version <id> --in-place` (no `--out`) recreated the
      deleted file at its original absolute source location with no prompt (nothing existed
      there yet), and a second `--in-place` restore over the now-existing file correctly refused
      without `--force`, matching `--out`'s existing guard. `show`/`diff`/`history` were also
      verified live (see this session's manual runs). The interactive version picker itself
      (`restore` with neither `--version` nor `--at`, real TTY) could not be driven end-to-end
      in this non-interactive automation environment (`Console.IsInputRedirected` is always
      `true` here); its code path was verified by (a) confirming the non-interactive fallback
      still reports the original clear error live, and (b) code review - it is a direct,
      standard use of Spectre.Console's `SelectionPrompt<T>`, the same library already relied on
      elsewhere in this codebase for the existing overwrite-confirmation prompt.
