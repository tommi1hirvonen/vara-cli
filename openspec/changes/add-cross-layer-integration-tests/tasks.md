...
- [x] 1.1 Create `tests/Vara.IntegrationTests/Vara.IntegrationTests.csproj`, following the same
...
- [x] 1.2 Add `<Project Path="tests/Vara.IntegrationTests/Vara.IntegrationTests.csproj" />` to
...
- [x] 2.1 In a new `tests/Vara.IntegrationTests/Backup/Fakes.cs`, add small, self-contained
...
- [x] 3.1 In a new `tests/Vara.IntegrationTests/Backup/BackupPipelineRealRepositoryTests.cs`, add
...
- [x] 3.2 In the same file, add a test that runs the pipeline twice against an unchanged source
...
- [x] 3.3 In the same file, add a test that runs the pipeline, then renames the source file
...
- [x] 4.1 In a new `tests/Vara.IntegrationTests/Retention/PruneServiceRealPortsTests.cs`, add a
  ...
  seed content via `StoreFromStream`, record snapshots directly against the real repository, run
  `Prune`, and assert both that the expired snapshot's manifest rows are gone from the real
  repository and that the unreferenced blob is actually deleted from disk (e.g. `File.Exists`
  against the blob's real path is false, or `HasContent` returns false). Verify the test passes.
- [x] 4.2 In the same file, add a test modeled on
  `PruneServiceTests.Prune_never_touches_files_currently_present_in_the_live_mirror`, but
  without seeding a mirror file via `PlaceAtMirrorPath` (which requires
  `ProbeHardlinkSupport` - excluded by design.md's Non-Goals as orthogonal hardlink-path
  surface). Instead assert the equivalent, mirror-independent guarantee directly: after pruning,
  content still referenced by the current (un-pruned) snapshot's manifest row survives
  (`HasContent` true) even though its earlier, superseded version's snapshot was eligible for
  removal, while the superseded version's own now-unreferenced content is actually deleted from
  disk. `PruneService.Prune` never touches the mirror directly (only blob storage and the
  manifest), so this preserves the same tested guarantee without the hardlink-probing
  precondition. Verify the test passes.
- [x] 4.3 In the same file, add a test modeled on
...
- [x] 5.1 In a new `tests/Vara.IntegrationTests/History/SnapshotHistoryServiceRealPortsTests.cs`,
...
- [x] 5.2 In the same file, add a test modeled on the mirror-containment guard (see
...
- [x] 5.3 In the same file, add a test asserting the existing-destination/overwrite behavior
...
- [x] 6.1 Run the full solution test suite (`dotnet test`) and verify all tests across all four
