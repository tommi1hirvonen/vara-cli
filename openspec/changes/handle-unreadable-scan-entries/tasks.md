...
- [x] 1.1 Add a `ScanFailure` record (relative path + reason: unreadable entry vs.
...
- [x] 1.2 Change `IFileSystemScanner.Scan` (or add a companion member) so a scan
...
- [x] 2.1 In `DirectoryFileSystemScanner.Walk`, widen the per-entry
...
- [x] 2.2 In `DirectoryFileSystemScanner.ToEntry`, widen the `File.GetAttributes`
...
- [x] 2.3 In `DirectoryFileSystemScanner.TryGetLinkTarget`, widen the catch the
      same way so a permission-denied reparse point cannot throw uncaught.
- [x] 2.4 In `DirectoryFileSystemScanner.Walk`'s outer directory-enumeration catch,
      ...
      mitigation).
- [x] 2.5 Verify `dotnet build` succeeds for `Vara.Infrastructure` after the above
      changes.

## 3. Pipeline integration

- [x] 3.1 Update `BackupPipeline.Run` to collect `ScanFailure`s produced during
      scanning (alongside the scanned entries consumed by `BackupDiffer.Diff`).
- [x] 3.2 Merge scan failures with `BackupExecutor`'s existing per-file failures
...
- [x] 3.3 Confirm (by reading `BackupRunSummaryFormatter.Format`) that a scan
...
- [x] 4.1 ~~Add a `DirectoryFileSystemScannerTests` case that denies read access to
      a single file...~~ **Dropped** - verified empirically that Windows'
      `File.GetAttributes`/`FileInfo` metadata queries do not enforce a per-file
      ACL deny, so this scenario cannot be reproduced via ACL manipulation. See
      design.md's "Verified limitation" note. The per-entry catch widening
      (tasks 2.1-2.3) is kept as defensive coding but has no dedicated test.
- [x] 4.2 Add a case that denies access to enumerate a subdirectory and verifies
...
- [x] 4.3 Add a `BackupPipeline`/`BackupExecutor`-level test (or extend an existing
...
- [x] 4.4 Run the full test suite (`dotnet test`) and verify all tests pass,
