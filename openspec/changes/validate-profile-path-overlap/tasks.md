...
- [ ] 1.1 Add a private static path-normalization/containment helper to `Profile` (trim trailing
...
- [ ] 1.2 Update `ProfileResolver.TryResolveFromWorkingDirectory` to pass
...
- [ ] 1.3 Add/extend `ProfileTests` (tests/Vara.Core.Tests/Configuration/ProfileTests.cs) covering:
...
- [ ] 2.1 In `YamlProfileConfigLoader.ParseProfile`, catch the `ArgumentException` thrown by the
...
- [ ] 2.2 Add/extend `YamlProfileConfigLoaderTests`
...
- [ ] 3.1 Confirm `ProfileResolverTests` (tests/Vara.Application.Tests/Profiles/ProfileResolverTests.cs)
...
- [ ] 3.2 Run the full solution test suite and verify all tests pass: `dotnet build Vara.slnx -c
