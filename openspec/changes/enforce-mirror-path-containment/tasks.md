...
- [ ] 1.1 Add `MirrorPathEscapesTargetRootException : IOException` to
...
- [ ] 2.1 In `src/Vara.Infrastructure/Storage/FileSystemContentStore.cs`, extract the
...
- [ ] 2.2 Add a private `ResolveMirrorPath(string mirrorRelativePath)` helper that combines
...
- [ ] 3.1 Replace `Path.Combine(_mirrorRoot, mirrorRelativePath)` in `PlaceAtMirrorPath` with a
...
- [ ] 3.2 Replace both `Path.Combine(_mirrorRoot, fromRelativePath)` and
      ...
      path escapes.
- [ ] 3.3 Replace `Path.Combine(_mirrorRoot, mirrorRelativePath)` in `RemoveFromMirror` with a call
...
- [ ] 3.4 Add a regression test reproducing the original finding end-to-end at the
...
- [ ] 4.1 Update the XML doc comments on `IContentStore.PlaceAtMirrorPath`, `MoveMirrorEntry`, and
...
- [ ] 5.1 In `tests/Vara.Application.Tests/Backup/BackupExecutorTests.cs`, add a test (using
      ...
      exception type.
- [ ] 5.2 In `tests/Vara.Infrastructure.Tests/FileSystem/DirectoryFileSystemScannerTests.cs`,
...
- [ ] 6.1 Run `dotnet build Vara.slnx --nologo -v quiet` and confirm the solution builds with no
      warnings or errors.
- [ ] 6.2 Run `dotnet test` for the full solution and confirm all tests pass, including every
