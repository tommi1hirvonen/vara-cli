...
- [ ] 1.1 Add `RestoreDestinationInMirrorException` and `DestinationExistsException` to
...
- [ ] 1.2 Register both new exception types in `TryGetFriendlyMessage`'s switch in
      `src/Vara.Cli/Composition/ErrorReporting.cs`, and verify the solution builds.

## 2. Content store predicates

- [ ] 2.1 Add `bool IsWithinMirror(string absolutePath)` and `bool TargetExists(string
...
- [ ] 2.2 Implement both members on `FileSystemContentStore`
...
- [ ] 3.1 Add an `overwrite` parameter (default `false`) to `SnapshotHistoryService.RestoreAsOf`
...
- [ ] 3.2 Update the XML doc comments on `RestoreAsOf`/`RestoreVersion` to document the two new
      exceptions, and verify the solution builds.

## 4. CLI: `--force` flag and interactive confirmation

- [ ] 4.1 Add a `--force` boolean option to `RestoreCommand`
      ...
      option.
- [ ] 4.2 When `--force` is not set, call the restore with `overwrite: false` first; on catching
...
- [ ] 4.3 If the user's answer to the prompt is affirmative (`y`/`yes`, case-insensitive),
...
- [ ] 4.4 Verify manually that a restore whose `--out` resolves inside the profile's
...
- [ ] 5.1 Implement `IsWithinMirror` and `TargetExists` on the in-memory `FakeContentStore`
...
- [ ] 6.1 Add `FileSystemContentStoreTests` cases (`tests/Vara.Infrastructure.Tests/Storage/
...
- [ ] 6.2 Add `SnapshotHistoryServiceTests` cases (`tests/Vara.Application.Tests/History/
...
- [ ] 6.3 Run `dotnet test` for `Vara.Core.Tests`, `Vara.Application.Tests`, and
