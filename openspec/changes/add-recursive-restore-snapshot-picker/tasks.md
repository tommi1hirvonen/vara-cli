...
- [x] 1.1 Add `GetFileHistoryUnderPrefix(string prefix)` to `ISnapshotRepository`
  ...
  most-recent-first.
- [x] 1.2 Implement it in `SqliteSnapshotRepository`
...
- [x] 1.3 Add/extend an integration test in
...
- [x] 2.1 Add a `DirectorySnapshotCandidate` (or similar) record to
...
- [x] 2.2 Add `SnapshotHistoryService.ListDirectorySnapshotCandidates(string directoryPath)`:
...
- [x] 2.3 Add `SnapshotHistoryService.ResolveDirectorySnapshot(string directoryPath, long
...
- [x] 3.1 Add the `--snapshot <long>` option to `RestoreCommand.Create`
...
- [x] 3.2 In `RunRecursiveRestore`, when `--snapshot` is given, resolve it via
  `ResolveDirectorySnapshot` and use its `StartedAt` as `asOf`, bypassing the picker.
- [x] 3.3 When neither `--at` nor `--snapshot` is given and the session is interactive (the
...
- [x] 3.4 Render each candidate row with directory-scoped delta stats and point-in-time totals
...
- [x] 3.5 Update the `--at`, `--version`, and new `--snapshot` option descriptions in
  `RestoreCommand.Create` to reflect the new behavior and mutual-exclusion rules.
- [x] 3.6 Extend `tests/Vara.Cli.Tests/Commands/RestoreCommandTests.cs` covering: `--snapshot`
...
- [x] 4.1 Confirm `openspec validate --strict` passes for this change once specs/design/tasks are
  all written.
- [x] 4.2 Update `README.md`/CLI help text (if the project documents `restore` usage there
