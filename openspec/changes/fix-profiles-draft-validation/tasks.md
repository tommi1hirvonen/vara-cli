...
- [x] 1.1 Make `SourceDraft.TryBuild` pass *copies* of its `Excludes`/`IncludeGlobs`/`ExcludeGlobs`
...
- [x] 1.2 Replace the live duplicate-name error text in `ProfileDraft.Revalidate` (currently
...
- [x] 1.3 Verify a unit test covers the spec scenario "Later source edits do not change an
...
- [x] 2.1 Change `ProfilesCommand.ApplySave` to take the profile to persist as an explicit
...
- [x] 2.2 Change the edit screen's `EditAction.Save` branch to call `draft.Revalidate(profiles)`
...
- [x] 2.3 Verify no remaining code path outside `ProfileDraft` reads `CurrentValidProfile` to
...
- [x] 3.1 In `ProfilesCommand.RunSourcesScreen`, call `draft.Revalidate(profiles)` immediately
...
- [x] 3.2 In `ProfilesCommand.RunEditScreen`, call `draft.Revalidate(profiles)` after
...
- [x] 3.3 Confirm the removal path (`SourceFieldAction.Remove`) still revalidates and that
...
- [x] 4.1 Change `ProfilesCommand.PromptOptionalInt`'s `int.TryParse` validation and `int.Parse`
...
- [x] 5.1 Add a `Vara.Cli.Tests` test reproducing the reported bug at the level the automated test
...
- [x] 5.2 Extend `ProfilesEndToEndTests` with a step that adds a second source, saves, and
...
- [x] 5.3 Run `dotnet test` and confirm the whole suite passes, with no regression in the existing
      profile-config, profile-draft, writer, backup, restore, or prune tests
- [x] 5.4 Build with `dotnet build Vara.slnx -c Debug --nologo -v quiet` and confirm it succeeds
