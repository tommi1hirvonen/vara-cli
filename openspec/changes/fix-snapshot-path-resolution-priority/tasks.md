...
- [x] 1.1 In `SnapshotPathResolver.TryResolve`, restructure the method so that when the
      ...
      checked
- [x] 1.2 Ensure the literal check and the absolute-source-path conversion (rule 3) still both run
      as fallbacks in both branches (cwd-inside-mirror and cwd-outside-mirror), preserving
      first-match-wins semantics
- [x] 1.3 Verify the method's existing XML doc comment (the `<list type="number">` describing
      resolution order) is updated to reflect the new conditional ordering

## 2. Tests

- [x] 2.1 Add a `SnapshotPathResolver` test: `currentDirectory` inside the mirror, `hasHistory`
...
- [x] 2.2 Add a regression test: `currentDirectory` outside the mirror, `hasHistory` matches the
      literal input, and verify the literal input still wins (unchanged from today)
- [x] 2.3 Add a regression test: `currentDirectory` inside the mirror, cwd-relative candidate does
      not match, literal input does match, and verify the literal input is still used as a
      fallback
- [x] 2.4 Add a regression test for the existing "absolute source path given directly" and
      "no interpretation matches" scenarios and verify they still pass unmodified
- [x] 2.5 Run the full `Vara.Application.Tests` (or equivalent) suite and verify all tests pass

## 3. Verification

- [x] 3.1 Manually reproduce the reported scenario: from inside a mirror subdirectory, run
