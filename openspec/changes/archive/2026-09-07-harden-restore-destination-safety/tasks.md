...
- [x] 1.1 Add a `GetFinalPathNameByHandleW` `LibraryImport` to `Vara.Infrastructure.Interop.Kernel32`,
...
- [x] 1.2 Implement a helper (e.g. in `Kernel32` or a small new internal type in
...
- [x] 1.3 Add unit tests: a plain path with no reparse points resolves unchanged; a path through a
...
- [x] 2.1 Update `FileSystemContentStore.IsPathWithinRoot` to resolve both `root` and `candidate`
      via the new real-path helper before the existing prefix comparison
- [x] 2.2 Add a `FileSystemContentStore` test: create a junction outside the mirror root pointing
      inside it, call `IsWithinMirror` on a path through that junction, and verify it now returns
      `true` (previously `false`)
- [x] 2.3 Add a regression test confirming `IsWithinMirror`/`ResolveMirrorPath` behavior for
      ordinary (non-reparse) paths is unchanged, and verify all existing containment tests still
      pass

## 3. Atomic `ExtractTo`

- [x] 3.1 Change `ExtractTo` to stage the copy under `_tempRoot` (reusing `CopyWithProgress`) and
...
- [x] 3.2 Add a test that simulates a failed copy (for example, a `CopyWithProgress` that throws
...
- [x] 3.3 Add a regression test confirming a normal successful restore over an existing
...
- [x] 4.1 Change `SnapshotPathResolver.TryResolve`'s signature to accept an
...
- [x] 4.2 Update every call site (`ShowCommand`, `HistoryCommand`, `DiffCommand`, `RestoreCommand`,
...
- [x] 4.3 Add a `SnapshotPathResolver` unit test covering the reparse-point-via-injected-predicate
...
- [x] 5.1 Run the full test suite (`dotnet test`) and verify all tests pass
- [x] 5.2 Manually create a junction pointing into a test profile's mirror and confirm
...
- [x] 5.3 Manually interrupt a restore over an existing large file (for example, by killing the
