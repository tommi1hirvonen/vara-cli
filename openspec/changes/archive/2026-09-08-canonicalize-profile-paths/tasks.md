...
- [x] 1.1 In `Source`'s constructor (`src/Vara.Core/Configuration/Profile.cs`), after the existing
...
- [x] 1.2 Add a test in `tests/Vara.Core.Tests/Configuration/ProfileTests.cs` (`SourceTests`)
  ...
  test passes.
- [x] 1.3 Add a test asserting that two source paths denoting the same location but differing only
  in separator style (e.g. `C:/Data` vs `C:\Data`) produce identical `source.Path` values, and
  verify the test passes. Also add a test confirming a single trailing separator is preserved
  as-is (e.g. `C:\Backups\vara\` stays distinct from `C:\Backups\vara` without one) -
  `Path.GetFullPath` does not strip it, and this change does not add extra trimming logic.

## 2. Canonicalize `Profile.TargetRoot`

- [x] 2.1 In `Profile`'s constructor (`src/Vara.Core/Configuration/Profile.cs`), after the existing
...
- [x] 2.2 Add a test in `tests/Vara.Core.Tests/Configuration/ProfileTests.cs` (`ProfileTests`)
...
- [x] 3.1 Verify (or add if missing) a test constructing a `Profile` whose target root and a source
...
- [x] 4.1 Run `dotnet test tests/Vara.Core.Tests` and verify all tests pass, including the new ones
  from tasks 1-3.
- [x] 4.2 Run `dotnet build Vara.slnx --nologo -v quiet` (or the project's standard whole-solution
