...
- [x] 1.1 In `DirectoryArgumentResolver`, replace the unconditional "empty prefix is always
      ...
      tracked.
- [x] 1.2 Update `The_mirror_root_itself_always_resolves` (or replace it) to reflect the new
...
- [x] 1.3 Add a unit test covering the case where the cwd (outside the mirror) source-maps to a
...
- [x] 1.4 Add/confirm a unit test that a non-root, non-blank directory argument (e.g. `subdir`)
...
- [x] 2.1 Update `BrowseCommand` to consume the resolver's fallback signal and print a brief,
...
- [x] 2.2 Verify `vara browse .` (or omitting the argument) from a source-tree cwd that does map
...
- [x] 3.1 Extract/align `RestoreCommand.IsDirectoryTracked` and `RunRecursiveRestore`'s directory
...
- [x] 3.2 Print the same kind of brief fallback message (using `RestoreCommand`'s existing
...
- [x] 3.3 Add a test asserting `restore --recursive .` from a source-tree cwd that does map to
...
- [x] 4.1 Run the full existing test suite for the affected projects
      (`dotnet test tests/Vara.Cli.Tests`) and confirm no regressions beyond the intentionally
      updated tests from section 1. (One pre-existing, unrelated failure -
      `ProfilesCommandTests.ApplyDelete_removes_only_the_matching_profile_and_writes_the_remaining_list`
      - reproduces identically on a clean checkout without this change's edits.)
- [x] 4.2 Manually verify (or add an integration test) that `browse`/`restore --recursive` with a
