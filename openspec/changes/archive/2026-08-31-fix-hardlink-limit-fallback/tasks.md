...
- [x] 1.1 Add an internal testing seam to `FileSystemContentStore` that lets a test force a
...
- [x] 1.2 Wrap the `Kernel32.CreateHardLink` call inside `PlaceAtMirrorPath` in a try/catch for
...
- [x] 1.3 Update `IContentStore.PlaceAtMirrorPath`'s doc comment to describe the per-placement
...
- [x] 2.1 Add a `FileSystemContentStoreTests` case that forces a single placement's hardlink
...
- [x] 2.2 Add a `FileSystemContentStoreTests` case verifying that after a forced-failure
...
- [x] 2.3 Add a `BackupExecutorTests` case (using the existing fake content store) confirming
...
- [x] 3.1 Run `dotnet test` for `Vara.Infrastructure.Tests` and `Vara.Application.Tests` and
