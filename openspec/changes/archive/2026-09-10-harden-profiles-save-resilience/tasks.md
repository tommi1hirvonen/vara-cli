...
- [x] 1.1 Add a `ProfileConfigWriteFailedException` to
...
- [x] 1.2 Wrap `YamlProfileConfigWriter.WriteProfiles`'s file I/O so that an `IOException`,
...
- [x] 1.3 Verify `ErrorReporting.TryGetFriendlyMessage` already classifies the new exception (it
...
- [x] 2.1 Change `ProfilesCommand.ApplySave` so the configuration file is written *before* the
...
- [x] 2.2 Apply the same write-then-commit ordering to `ProfilesCommand.ApplyDelete`, and verify a
      test with a throwing writer asserts the profile is still present in the in-memory list
- [x] 2.3 Catch `ProfileConfigWriteFailedException` in the edit screen's Save action: report it
      ...
      any field
- [x] 2.4 Catch `ProfileConfigWriteFailedException` in the delete flow: report it and return to
      the main menu with the profile still listed; verify a test asserts the profile remains in
      the list after a failed delete
- [x] 2.5 Verify by inspection that no `WriteProfiles` call in `ProfilesCommand` can still unwind
      past `RunMenu` into `ErrorReporting.Run` (grep for `WriteProfiles` in `src/Vara.Cli`)

## 3. Ctrl+C exits the profiles editor

- [x] 3.1 Thread the shared `GracefulCancellation.TokenSource.Token` into
      ...
      solution builds
- [x] 3.2 Register a callback on that token at command entry (after the interactive-terminal
...
- [x] 3.3 Verify by test that the cancellation callback performs no write: assert that invoking
...
- [x] 4.1 Flush the temporary file's contents to durable storage before it is closed and swapped
      ...
      still pass
- [x] 4.2 Verify by inspection that the flush happens before the `File.Replace`/`File.Move` swap,
      not after, so the ordering the atomicity guarantee depends on is preserved

## 5. Cross-cutting verification

- [x] 5.1 Run `dotnet test` and confirm the whole suite passes, with no regression in the existing
      profile-config, writer, backup, restore, prune, or Ctrl+C/`GracefulCancellation` tests
- [x] 5.2 Build with `dotnet build Vara.slnx -c Debug --nologo -v quiet` and confirm it succeeds
      with `PublishAot=true` still set on `Vara.Cli`
- [x] 5.3 Manually verify the two-press Ctrl+C behavior of `backup`/`prune`/`check` is unchanged
