...
- [x] 1.1 Move `ProfileNameUniqueness` from `src/Vara.Application/Profiles/ProfileNameUniqueness.cs`
      ...
      `Vara.Application`
- [x] 1.2 Move `tests/Vara.Application.Tests/Profiles/ProfileNameUniquenessTests.cs` to
...
- [x] 2.1 Update `YamlProfileConfigLoader` to reference the helper from `Vara.Core.Configuration`
...
- [x] 2.2 Update `ProfileDraft` to reference the helper from `Vara.Core.Configuration` and verify
      the existing `ProfileDraftTests` uniqueness tests pass unchanged

## 3. Restore the layering

- [x] 3.1 Remove the `<ProjectReference Include="..\Vara.Application\Vara.Application.csproj" />`
...
- [x] 3.2 Verify `Vara.Infrastructure`'s only project reference is `Vara.Core`, and that no file
      under `src/Vara.Infrastructure` still contains `using Vara.Application` (grep to confirm)

## 4. Verification

- [x] 4.1 Build with `dotnet build Vara.slnx -c Debug --nologo -v quiet` and confirm it succeeds
      with `PublishAot=true` still set on `Vara.Cli`
- [x] 4.2 Run `dotnet test` and confirm the whole suite passes with no assertion changed by this
