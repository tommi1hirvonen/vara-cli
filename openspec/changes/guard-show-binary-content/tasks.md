...
- [ ] 1.1 Add a `ShowBinaryContentException` (single-sided sibling of `DiffBinaryContentException`)
...
- [ ] 1.2 Add a `forceBinary` parameter to `SnapshotHistoryService.ShowVersion`; when `false` and
...
- [ ] 1.3 Add unit tests: showing text content behaves unchanged; showing binary content with
...
- [ ] 2.1 Add a `--force-binary` boolean option to `ShowCommand`
- [ ] 2.2 Pass `Console.IsOutputRedirected || forceBinary` as `ShowVersion`'s `forceBinary`
      argument
- [ ] 2.3 Add a `ShowCommand` test (or extend an existing one) covering: binary content refused
...
- [ ] 3.1 Run the full test suite and verify all tests pass
- [ ] 3.2 Manually run `vara show` against a known binary-tracked file (a) with output attached to
