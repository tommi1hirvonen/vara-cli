...
- [ ] 1.1 In `BackupExecutor.ExecuteTransfer`, after `contentStore.StoreFromStream` and
      `contentStore.PlaceAtMirrorPath` complete, re-stat `operation.SourceAbsolutePath` (size and
      `LastWriteTimeUtc`) and compare against `operation.Size`/`operation.SourceModifiedAt`
- [ ] 1.2 On a match, record the manifest entry exactly as today; verify via unit test that
      behavior for an unchanged file is unaffected
- [ ] 1.3 On a mismatch, route to the same failure handling used for
...
- [ ] 1.4 Handle the re-stat call itself throwing (for example the file was deleted immediately
      after being read) by treating that as a failure too, and verify with a unit test

## 2. Tests

- [ ] 2.1 Add a `BackupExecutor` test that modifies a file's content and modification time between
...
- [ ] 2.2 Add a test verifying such a file is re-selected for transfer by the incremental change
      detection logic on a subsequent run (recorded manifest state is unchanged, so the file's
      current stat still differs from it)
- [ ] 2.3 Add a regression test confirming a normal, unmodified-during-transfer file still records
      its manifest entry with the scan-time modification time exactly as before
- [ ] 2.4 Run the full `Vara.Application.Tests` (or equivalent) suite and verify all tests pass
