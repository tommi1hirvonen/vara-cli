...
- [ ] 1.1 In `DirectoryFileSystemScanner.Walk`, before recursing into a plain subdirectory, compute
...
- [ ] 1.2 Ensure a reparse point (symlink/junction) is still reported exactly as today even when
...
- [ ] 1.3 Confirm `IsExcluded` needs no changes (only its call site moves) and verify existing
      `IsExcluded` unit tests still pass unmodified

## 2. Tests

- [ ] 2.1 Add a scanner test with an excluded directory containing an unreadable nested file
      (simulate an `UnauthorizedAccessException` on enumeration) and verify no `ScanFailure` is
      recorded for it
- [ ] 2.2 Add a scanner test asserting a mocked/instrumented filesystem's excluded directory is
      never enumerated (zero `Directory.EnumerateFileSystemEntries` calls under it)
- [ ] 2.3 Add a regression test confirming non-excluded directories and file-level glob
      include/exclude filtering behave exactly as before
- [ ] 2.4 Run the full `Vara.Infrastructure.Tests` (or equivalent scanner test project) suite and
      verify all tests pass

## 3. Verification

- [ ] 3.1 Manually configure a profile with a large excluded directory (for example pointing
