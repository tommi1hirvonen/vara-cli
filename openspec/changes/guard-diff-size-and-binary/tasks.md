...
- [x] 1.1 Add `DiffContentTooLargeException` and `DiffBinaryContentException` to
...
- [x] 2.1 In `src/Vara.Application/History/SnapshotHistoryService.cs`, add a size check in
...
- [x] 2.2 Add a binary-content check in `OpenVersionsForDiff`, run only after both sizes pass the
  ...
  actually rewinds).
- [x] 2.3 Verify size and binary guards compose in the stated order: a case with one side
...
- [x] 3.1 Add `DiffContentTooLargeException` and `DiffBinaryContentException` to the recognized
...
- [x] 4.1 Add an integration-level test (alongside
...
- [x] 4.2 Run the full test suite (`dotnet test`) and confirm all tests pass, including the new
