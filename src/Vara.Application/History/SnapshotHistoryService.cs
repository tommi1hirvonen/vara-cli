...
public sealed class SnapshotHistoryService(ISnapshotRepository repository, IContentStore contentStore)
{
    ...
    public void RestoreAsOf(string relativePath, DateTimeOffset asOf, string destinationPath)
    {
    ...
    public void RestoreVersion(string relativePath, long versionId, string destinationPath)
    {
        var history = repository.GetFileHistory(relativePath);
        if (history.Count == 0)
        {
